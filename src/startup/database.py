...
def retry_sql_query(retries=3, delay=1):
    """Retry sql query in case of bad connections. TODO: Implement more specific error catching.
...
async def run_query(sql_query_statement: str, params: list = None, query_context: str = None, extra_tabs: int = 0, exec_many: bool = False) -> list[dict]:
    """Run a query against database pool.
    ...
    """
    print('\n' + '\t'*extra_tabs + '+'*10 + f' Running Select Query {query_context if query_context else ""} ' + '+'*10)
    ...
    try:
        ...
        error_msg = f"Error occurred during database query: {e}"
        # Log more context about the query that failed
        error_details = {
            "query": sql_query_statement,
            "params": params,
            "context": query_context,
            "error": str(e),
            "error_type": type(e).__name__
        }
        print('\t'*extra_tabs + error_msg)
        print('\t'*extra_tabs + f"Query details: {error_details}")
        # Consider using a proper logging library instead of print
        # logging.error(error_msg, extra=error_details)
...
async def execute_store_procedure(stored_procedure_name: str, params: dict, extra_tabs: int = 0) -> list[dict]:
    """Execute Parameterized Stored Proc.
